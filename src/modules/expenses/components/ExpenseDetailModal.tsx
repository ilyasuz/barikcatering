...
export function ExpenseDetailModal({ isOpen, onClose, record }: ExpenseDetailModalProps) {
      ...
      <div className={`modal-content ${isOpen ? 'open' : ''}`} onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{
          ...
          <h2 style={{ fontSize: '18px', margin: 0, display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            {t('expenses.detail.title', 'Gider Detayı')}
          </h2>
        ...
        <div style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            <div>
              <div style={{ fontSize: '13px', color: 'var(--text-muted)', marginBottom: '4px' }}>{t('expenses.detail.invoiceNo', 'Belge / Fatura No')}</div>
              <div style={{ fontSize: '16px', fontWeight: 600 }}>{record.invoiceNo}</div>
            </div>
            <StatusBadge status={record.status} label={record.status === 'completed' ? t('common.completed', 'Ödendi') : record.status === 'pending' ? t('common.pending', 'Bekliyor') : t('common.overdue', 'Gecikmiş')} />
          </div>
          ...
          <div style={{ display: 'flex', gap: '12px', padding: '16px', backgroundColor: 'var(--bg-secondary)', borderRadius: '8px', alignItems: 'center' }}>
            ...
            <div>
              <div style={{ fontSize: '12px', color: 'var(--text-muted)' }}>{t('expenses.detail.supplier', 'Tedarikçi / Kurum / Personel')}</div>
              <div style={{ fontSize: '15px', fontWeight: 500 }}>{record.supplierName}</div>
            </div>
          </div>

          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
            <div>
              <div style={{ fontSize: '12px', color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: '4px' }}><Calendar size={14} /> {t('expenses.detail.date', 'İşlem Tarihi')}</div>
              <div style={{ fontSize: '14px', marginTop: '4px' }}>{record.date}</div>
            </div>
            <div>
              <div style={{ fontSize: '12px', color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: '4px' }}><Calendar size={14} /> {t('expenses.detail.dueDate', 'Son Ödeme Tarihi')}</div>
          ...
          <div>
            <div style={{ fontSize: '12px', color: 'var(--text-muted)', marginBottom: '4px', display: 'flex', alignItems: 'center', gap: '4px' }}><Tag size={14} /> {t('expenses.detail.descriptionCategory', 'Açıklama & Kategori')}</div>
            <div style={{ fontSize: '14px' }}>{record.description}</div>
            <div style={{ marginTop: '8px' }}>
              <span style={{ padding: '2px 8px', borderRadius: '12px', fontSize: '12px', backgroundColor: 'rgba(239,68,68,0.1)', color: '#EF4444', border: '1px solid rgba(239,68,68,0.2)' }}>
                {record.category || t('expenses.detail.noCategory', 'Kategori Yok')}
              </span>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: '12px', color: 'var(--text-muted)', marginBottom: '4px', display: 'flex', alignItems: 'center', gap: '4px' }}><CreditCard size={14} /> {t('expenses.detail.paymentSummary', 'Ödeme Özeti')}</div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', maxWidth: '200px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '12px' }}>
                  <span style={{ color: 'var(--text-muted)' }}>{t('expenses.detail.paid', 'Ödenen:')}</span>
                  <span>{new Intl.NumberFormat('tr-TR').format(record.paidAmount || 0)} {record.currency}</span>
                </div>
                <div style={{ width: '100%', height: '6px', backgroundColor: 'var(--bg-tertiary)', borderRadius: '3px', overflow: 'hidden' }}>
                  <div style={{ width: `${percent}%`, height: '100%', backgroundColor: percent === 100 ? '#10b981' : '#F59E0B' }} />
                </div>
              </div>
            </div>
            <div style={{ textAlign: 'right' }}>
              <div style={{ fontSize: '12px', color: 'var(--text-muted)', marginBottom: '4px' }}>{t('expenses.detail.totalAmount', 'Toplam Tutar')}</div>
            ...
            <>
              ...
              <div>
                <div style={{ fontSize: '13px', fontWeight: 600, color: 'var(--text-primary)', marginBottom: '12px', display: 'flex', alignItems: 'center', gap: '6px' }}>
                  <Wallet size={16} color="#10b981" /> {t('expenses.detail.paymentHistory', 'Ödeme Geçmişi')}
                </div>
                    ...
                    <div key={idx} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 12px', backgroundColor: 'var(--bg-secondary)', borderRadius: '6px', borderLeft: '3px solid #10b981' }}>
                      <div>
                        <div style={{ fontSize: '13px', fontWeight: 500 }}>
                          {payment.method === 'cash' ? t('common.cash', 'Nakit') : payment.method === 'bank_transfer' ? t('common.bankTransfer', 'Banka Transferi') : payment.method === 'credit_card' ? t('common.creditCard', 'Kredi Kartı') : t('common.check', 'Çek')}
        ...
        {record.attachments && record.attachments.length > 0 && (
          <div style={{ padding: '0 24px 24px 24px' }}>
            <div style={{ fontSize: '12px', color: 'var(--text-muted)', marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '4px' }}>
              <ImageIcon size={14} /> {t('expenses.detail.attachments', 'Ekli Dosyalar (Dekont / Fiş / Fatura)')}
            </div>
